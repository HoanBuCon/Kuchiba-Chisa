...
| Ngày cập nhật yêu cầu gần nhất | 2026-09-12 |
...
| SRS-CHG-20260906-RAG05-002 | 2026-09-06T03:47:15+07:00 | User/project owner, phê duyệt trực tiếp trong phiên triển khai | `RAG-05`, `TD-037` | Defer production-equivalent remote reranker ablation với Voyage `rerank-3-lite` ở Tier-0; giữ nguyên Voyage support và chọn Jina hosted API `jina-reranker-v3.5` làm candidate prepaid/token-package tiếp theo. | Tier-0 Voyage khoảng 3 RPM/10,000 TPM làm full 83-case run phát sinh pacing dài và rolling-window 429; đây là hạn chế execution tier, không phải lỗi retrieval/architecture và không được coi là PASS hoặc quality FAIL. Exit condition: nâng Voyage lên quota thực dụng rồi rerun benchmark frozen, hoặc benchmark một provider prepaid/token-package được phê duyệt trên cùng production-equivalent candidate path. |
| SRS-CHG-20260906-RAG05-003 | 2026-09-06T04:33:38+07:00 | User/project owner, yêu cầu đánh giá và đóng debt theo exit condition đã duyệt | `TD-037`, `RAG-05` | Đóng `TD-037` theo exit path (b): approved alternative remote reranker Jina `jina-reranker-v3.5` đã hoàn tất formal benchmark trên cùng frozen production-equivalent candidate path. | `reports/RAG05_Staging_Jina_Ablation.{md,json}`: fingerprint `01cababd2e9912a1b435869afc500dc44d727d045f6bafe868c3be4bc6004976`, 83/83 validated responses, 0 failure/fallback, Hit@5 0.975309, MRR@10 0.909465, provider HTTP p95 413.085 ms, namespace unchanged. Lịch sử Voyage Tier-0 vẫn được giữ; closure này chỉ giải quyết deferred provider verification, không đánh dấu toàn bộ `RAG-05` PASS. |
| SRS-CHG-20260906-P1-001 | 2026-09-06T19:21:30.0340858+07:00 | User/project owner `/goal` yêu cầu hoàn tất và đóng toàn bộ P1 khi có evidence | `TD-036`, `RAG-03`–`RAG-06`, `SAFE-01`, `SAFE-02`, `ING-01`–`ING-03` | Ghi trạng thái P1 PASS sau khi các DoD độc lập và release gates đều đạt; không thay đổi threshold hoặc scope P2/P3. | `reports/P1_Formal_Closure_Assessment.md`: full pytest 567 passed; mypy 274 files; changed-lines Ruff PASS; Ruff debt 2,866 ≤ baseline 2,914; isolated PostgreSQL/Redis/Qdrant and Linux Docker verification PASS; protected prompt hashes 5/5 unchanged. |
| SRS-CHG-20260912-BE02-001 | 2026-09-12T18:03:44+07:00 | User/project owner `/goal` yêu cầu triển khai và chỉ đóng `BE-02` khi có evidence | `BE-02`, `NFR-REL-002`, `NFR-REL-003`, `NFR-PERF-009`, `TD-017`, `TD-018`, `TD-022` | Ghi nhận typed capability-aware LLM gateway, provider/model/purpose/capability breaker isolation, provider bulkhead, bounded classified retry, compatible fallback, shared two-call budget, explicit vision degradation và startup/readiness validation đã đạt DoD. Không thay đổi provider strategy, threshold hoặc prompt được bảo vệ. | `reports/BE02_LLM_Gateway_Verification.md`: focused 101 PASS; isolated Linux PostgreSQL/Redis/Qdrant 533 PASS; host và Linux mypy 290 files PASS; changed-lines/new-file Ruff PASS; debt 2,823 ≤ baseline 2,914; protected prompt audit PASS. |
| SRS-CHG-20260912-TD025-001 | 2026-09-12T19:28:51+07:00 | User/project owner `/goal` yêu cầu triển khai và chỉ đóng `TD-025` khi có evidence | `TD-025`, `NFR-REL-004`, `NFR-PERF-009`, `RAG-06`, `BE-02`, `BE-03` | Ghi nhận lore-answer cache v2 với deterministic corpus/generation/prompt/grounding/authorization identity, strict grounded citation receipt, fail-safe legacy handling và alias revalidation trước atomic publication. Không thay đổi requirement, threshold, provider policy hoặc protected prompt. | `reports/TD025_Lore_Answer_Cache_Verification.md`: focused 112 PASS; host unit 542 PASS; isolated Linux PostgreSQL/Redis/Qdrant 558 PASS; Alembic drift, mypy 292 files, changed-lines/new-file Ruff, debt ratchet 2.817 ≤ 2.914, pip/compile/diff và protected prompt audit đều PASS. |
...
Chiến lược provider của `RAG-05` giữ abstraction `ICrossEncoderReranker` với Voyage, Jina và deterministic fallback. Production-equivalent Voyage `rerank-3-lite` verification từng được ghi nhận tại `TD-037` ở trạng thái **DEFERRED/OPEN** vì Tier-0 khoảng 3 RPM/10,000 TPM không phù hợp để hoàn tất full 83-case formal run trong thời gian thực dụng. `TD-037` hiện **RESOLVED** qua exit path Jina trên cùng frozen configuration; lịch sử deferment vẫn được giữ và không chứng minh lỗi retrieval hoặc architecture.

Hosted Jina API `jina-reranker-v3.5` đã hoàn tất ablation với cùng approved 83-case golden set, production-equivalent candidate path, retrieval configuration và candidate count, không post-hoc tuning. Kết quả benchmark không tự động thay đổi production provider default. Chỉ user/project owner được phép tạo key, mua token package hoặc thay đổi billing; auto top-up **MUST NOT** được bật nếu chưa có phê duyệt riêng. Prepaid balance không được coi là hard budget cap nếu provider không bảo đảm semantics đó; application vẫn PHẢI có request/token/cost budget, timeout và fail-closed deterministic hybrid fallback.
...
| TD-017 | High — **RESOLVED 2026-09-12** | `assessor.py`; historical shared `circuit_breaker.py:57-58`; `reports/BE02_LLM_Gateway_Verification.md` | Context assessor từng là extra LLM gate và fail-open aligned khi lỗi; một global in-process breaker từng dùng cho mọi LLM purpose. | Global breaker đã được loại bỏ. Optional assessor dùng deterministic budget-preserving decision khi cần giữ call cuối; breaker/bulkhead được cô lập theo provider/model/purpose/capability với closed/open/half-open và bounded probe tests. Full shared operational metrics vẫn thuộc `OPS-02`. |
| TD-018 | High — **RESOLVED 2026-09-12** | LLM adapters; `app/application/llm_gateway.py`; `app/infrastructure/llm/gateway_factory.py`; `reports/BE02_LLM_Gateway_Verification.md` | Trước `BE-02` không có cross-provider failover hoặc capability registry; streaming vision có thể mất image semantics. | Explicit capability/purpose registry và deterministic compatible fallback matrix đã triển khai. Vision/tool/stream/structured combinations có negative tests; vision không bao giờ fallback sang text-only và unsupported streaming vision trả degraded rõ ràng. |
...
| TD-020 | High | Background stages/tasks; legacy settings `CELERY_*` | **CLOSED by BE-01.** Trước BE-01, memory extraction, private/community summary và visual-memory ingestion dùng task `asyncio` in-process nên có thể mất khi API crash/redeploy; Celery config không được dùng. | PostgreSQL transactional outbox + durable worker hiện cung cấp typed reference payload, unique idempotency key, atomic `SKIP LOCKED` claim, renewable lease/fencing, bounded retry/backoff, DLQ, audited replay và graceful drain. Minimal content-free queue snapshot được giữ cho `OPS-02`; shared-state/cache coordination rộng hơn vẫn thuộc `BE-03`. Evidence: `reports/BE01_Durable_Workers_Verification.md`. |
...
| TD-022 | Medium — **RESOLVED 2026-09-12** | `main.py`; `routes/health.py`; `gateway_factory.validate_llm_configuration`; `reports/BE02_LLM_Gateway_Verification.md` | Production startup từng kiểm tra `startup_errors` trước khi thêm lỗi thiếu LLM API key và có thể start dù provider chưa cấu hình. | Preflight provider/model/key/purpose/fallback/capability được gom trước production decision; `/ready` công bố `llm_config` riêng, không gọi provider tốn kém ở startup và không biến transient Internet outage thành permanent process-start failure. |
...
| TD-024 — **CLOSED by DB-01** | Medium | `database/engine.py`; Alembic; Discord database adapter | Trước DB-01, app/bootstrap và Discord có runtime DDL độc lập, làm schema ownership/drift khó kiểm soát. | Alembic hiện là owner PostgreSQL duy nhất; migration `7a8c9d0e1f2b` tiếp quản schema Discord không xóa row; startup chỉ verify revision; CI chạy drift check. Evidence: `reports/DB01_Schema_Ownership_Verification.md`. |
| TD-025 — **RESOLVED 2026-09-12** | Medium | `cache_stage.py`; `cache_update_stage.py`; `lore_answer_cache.py`; `qdrant_service.py` | Trước TD-025, cache chỉ chạy khi intents đúng `[LORE]`; key global không có corpus/model/prompt/ACL version và value chỉ là raw answer. | Typed `lore-answer-v2` contract nay bind normalized query với authoritative lore alias targets, BE-02 generation policy, prompt/grounding schema versions và principal/tenant/channel scope; strict citation ACL receipt, safe legacy miss, expiry và pre-write alias revalidation ngăn stale/unauthorized reuse. Evidence: `reports/TD025_Lore_Answer_Cache_Verification.md`. |
| TD-026 — **CLOSED by BE-03** | Medium | `persistence_stage.py`; user/private-summary cache projections | Trước BE-03, Redis state được write-through trước route transaction commit; lỗi sau đó có thể để cache thấy state chưa commit. | PostgreSQL canonical state nay được cập nhật atomic; transaction tạo durable outbox projection, worker chỉ publish cache sau commit bằng revision-aware Redis CAS. Rollback, stale/replayed event và cache recreation có isolated PostgreSQL/Redis tests. Evidence: `reports/BE03_Concurrency_State_Verification.md`. |
| TD-027 — **CLOSED by BE-03** | High | Community guild/channel shared state | Trước BE-03, lock theo user trong khi ambient/topic là guild/channel shared; concurrent speakers có lost update/race. | Redis Lua atomic transition theo guild+channel, guild-scoped ambient delta, event idempotency TTL, immutable summary snapshot và source-revision CAS ngăn lost update/replay/stale publish. Evidence: `reports/BE03_Concurrency_State_Verification.md`. |
...
| TD-035 — **CLOSED by BE-03** | Medium | `retriever_image_memory.py`; `vision_security.py`; `maintenance_tasks.py` | Trước BE-03, orphan cleanup và storage quota tạo raw `asyncio.create_task` không được lifecycle tracker quản lý; có thể mất lỗi/task khi shutdown. | Node-local maintenance supervisor cung cấp keyed coalescing, bounded retry, content-free telemetry và graceful drain/cancel. Orphan deletion revalidate exact point/payload fingerprint và file absence trước server-side filtered delete; quota failures được retry thay vì bị nuốt. Evidence: `reports/BE03_Concurrency_State_Verification.md`. |
...
| TD-037 | Medium — **RESOLVED 2026-09-06T04:33:38+07:00** | `RAG-05`; historical deferment: `reports/RAG05_Staging_Voyage_Ablation.{md,json}`; closure: `reports/RAG05_Staging_Jina_Ablation.{md,json}`; staging version `rag05eval_01cababd2e_20260905192227` | Historical problem preserved: production-equivalent Voyage `rerank-3-lite` ablation was DEFERRED/OPEN because Tier-0 khoảng 3 RPM/10,000 TPM made the full 83-case run impractical and caused rolling-window boundary 429. Golden set, retrieval configuration and staging candidate path remained frozen; no architecture/retrieval defect, PASS or quality FAIL was inferred from the deferment. | Exit path (b) satisfied: approved Jina `jina-reranker-v3.5` completed the same frozen production-equivalent evaluation with fingerprint `01cababd…04976`; 83/83 validated, 0 failure/fallback, Hit@5 0.975309, MRR@10 0.909465, HTTP p95 413.085 ms and unchanged namespace. Human-reviewed top-5 relevance then established context precision 1.0. `TD-037` and overall `RAG-05` are closed. |
| TD-038 | High — **RESOLVED 2026-09-06T18:39:10+07:00** | `RAG-06`, `FR-RAG-008`, `FR-RAG-009`, `SEC-RAG-007`, `NFR-RAG-001`, `NFR-RAG-002`, `NFR-RAG-005`, `NFR-RAG-007`; pre-remediation `rag06_final_answer_review_v1.*`; reviewed remediation `rag06_final_answer_review_v2.*`; `reports/RAG06_Grounding_Remediation_Verification.md`; `reports/RAG06_Final_Acceptance.md` | V1 exposed missing citations, unsupported embellishment, causal errors and zero useful delivery. Code-bound remediation enforces a typed DeepSeek function contract, server-owned claim/citation binding, deterministic evidence-quote resolution and safe abstention. Project owner `HoanBuCon` approved all 38 frozen v2 cases without changing generated output, evidence, retrieval/reranker configuration, corpus/index or protected prompts. | Human-reviewed evidence: faithfulness 44/44 = 1.0 with zero critical unsupported claims; answer relevance 31/36 = 0.861111 (three partial and two false abstentions are not hidden); citation correctness 64/64 = 1.0; approved no-answer abstention precision 2/2 = 1.0 with wide Wilson CI. `RAG-06` passes; false-abstention and raw-wikitext/readability findings remain non-blocking debt. |
...
Trạng thái kiểm soát P1 tại 2026-09-06: **PASS**. Toàn bộ task P1 trong bảng
6.2 đã có evidence độc lập và các release gate bắt buộc đã đạt; receipt chi tiết
được lưu tại `reports/P1_Formal_Closure_Assessment.md`. Trạng thái này không suy
diễn PASS cho P2/P3 và không xóa các limitation/debt được giữ trong receipt.
...
| TD-036 Legacy Ruff debt ratchet | P1, incremental — **PASS** | MLOps/Code health | Giảm baseline 3,861 theo batch nhỏ có review; ưu tiên `F821`/rule semantic-security-correctness, rồi active modules, import hygiene và formatting. Không bulk-autofix; mỗi batch phải có Ruff count trước/sau, regression phù hợp và không tăng baseline. | `pyproject.toml`, CI lint tooling, module theo inventory |
| RAG-03 Evidence model | P1 — **PASS** | RAG | Typed evidence with provenance/ACL/version/scores propagated end-to-end | retrievers, pipeline context, context builder, schemas |
| RAG-04 True hybrid search | P1 — **PASS** | RAG | Dense+sparse/BM25, parallel with timeout, calibrated RRF; ablation report | RAG pipeline, new sparse service/index |
| RAG-05 Cross-encoder reranking | P1 — **PASS** | RAG | Giữ port `ICrossEncoderReranker`; triển khai adapter API primary, adapter local chỉ khi artifact được approve/provision, và deterministic fallback. Benchmark Voyage (`rerank-3-lite`, `rerank-3`), Jina (`jina-reranker-v3.5`, `jina-reranker-v2-base-multilingual`) và Cohere (`Rerank v3.5`) trên cùng golden set. API rerank chỉ lore/factual/complex public/approved, top 10–15; timeout/429/outage fallback calibrated dense+sparse RRF (+ entity/heuristic feature nếu có), telemetry `degraded`, confidence phù hợp, citation/grounding/security không giảm. Không auto-download model local request-time; không hard-code local-only/API-only. DoD: ablation cải thiện so với baseline, remote provider HTTP latency đạt `NFR-PERF-006` theo đúng measurement semantics, timeout/fallback/privacy/adversarial tests pass. Production-equivalent Jina evidence đạt Hit@5 0.975309, MRR@10 0.909465, provider HTTP p95 413.085 ms và 83/83 validated responses không fallback; vì vậy exit condition của `TD-037` đã đạt. Focused provider-boundary evidence tại `reports/RAG05_Privacy_Fallback_Verification.md` xác nhận public-only remote payload, PII/secret redaction và deterministic observable fallback cho privacy rejection, timeout, 429, invalid response và network failure. Human-reviewed top-5 annotation tại `data/evaluations/drafts/rag05_context_precision_review_v1.{json,md}` bao phủ 405/405 item của 81 answerable cases, loại 2 abstention cases khỏi denominator và đạt context precision 1.0 ≥ 0.75; reviewer đồng thời ghi nhận raw-wikitext/chunking noise là vấn đề chất lượng dữ liệu riêng. `SAFE-01`, `SAFE-02` và `RAG-06` vẫn có evidence/acceptance độc lập; PASS này không suy diễn trạng thái của các task đó. | `app/domain/interfaces/reranker.py`, `app/infrastructure/rag/`, `retriever_lore.py`, dependencies, evaluation tests |
| TD-037 RAG-05 Tier-0 provider verification | P1 — **RESOLVED** | RAG/MLOps | Historical Voyage Tier-0 deferment retained. Exit path (b) completed without retrieval/reranker tuning: formal Jina `jina-reranker-v3.5` ablation reused the frozen approved 83-case golden set, production-equivalent staging candidate path, dense/sparse/RRF configuration and candidate count. Together with approved context-precision and boundary evidence, this closes `RAG-05`. | `reports/RAG05_Provider_Transition_Plan.md`, `reports/RAG05_Staging_Jina_Ablation.{md,json}` |
| RAG-06 Grounding/citations | P1 — **PASS (`TD-038` resolved)** | RAG/Safety | Typed claim/evidence output, server-owned citations and safe abstention are implemented. Frozen v2 human review by `HoanBuCon` covers 38/38 cases and achieves faithfulness 1.0 with zero critical unsupported claims, answer relevance 0.861111, citation correctness 1.0 and approved no-answer abstention precision 1.0. Wilson intervals, two false abstentions and the two-case abstention limitation remain explicit in `reports/RAG06_Final_Acceptance.md`; no threshold is waived. | generation/output guard, DeepSeek adapter, `rag06_final_answer_review_v2.*`, `reports/RAG06_Final_Acceptance.md` |
| SAFE-01 Injection/jailbreak guard | P1 — **PASS** | Security/AI | Multi-layer input/RAG/image guard, leakage canary, VN/EN adversarial suite | guardrail package, ingestion, chat pipeline |
| SAFE-02 PII/consent policy | P1 — **PASS** | Security/Backend | Classification, masking, memory consent/retention and privacy tests | schemas, memory service, logs/providers |
| ING-01 Unified ingestion DAG | P1 — **PASS** | RAG/Data | The application `IngestionOrchestrator` and governed `run-dag` CLI are the sole executable standard ingestion path. Reviewed fixture parity covers source revision, factual canonical text, deterministic identity/span, ACL/provenance and canonical state/acknowledgement receipts. `MasterIngestionPipeline`, direct file/SQLite/Qdrant commands and destructive alternate ingestion scripts were removed only after parity; intentional representation differences and raw-wikitext readability debt remain explicit in `reports/ING01_Canonical_Parity_Verification.md`. | application DAG, canonical CLI, operator CLI, parity/security tests |
| ING-02 Atomic corpus version | P1 — **PASS** | RAG/Data | Staging dense+sparse+parents, checksums/ACL/golden gate, alias swap/rollback | ingestion storage, Qdrant, PostgreSQL |
| ING-03 Source governance | P1 — **PASS** | Security/Data | Registry, trust/license/quarantine, poison/PII scan, curator approval/audit | ingestion models/admin APIs |
| BE-01 Durable async workers | P2 — **PASS** | Backend | PostgreSQL transactional outbox commit cùng source state; worker riêng với atomic claim, renewable lease/fencing, bounded retry+jitter, DLQ/audited replay, typed privacy-bounded payload và graceful drain. Restart, concurrent claim, stale lease, duplicate enqueue/delivery và deterministic side-effect tests đạt; Alembic migration `8b9d0e1f2a3c`, drift check và isolated Linux Compose đạt. Evidence: `reports/BE01_Durable_Workers_Verification.md`. | background stages, `app/worker.py`, PostgreSQL |
| BE-02 LLM gateway/failover | P2 — **PASS** | Backend/RAG | Typed capability/purpose registry; breaker cô lập theo provider/model/purpose/capability; provider bulkhead; classified bounded retry; compatible fallback; phase/deadline timeout; shared max-two-call budget; explicit vision degradation; startup/readiness validation. Evidence: `reports/BE02_LLM_Gateway_Verification.md`. | `app/domain/interfaces/llm_provider.py`, `app/application/llm_gateway.py`, `app/infrastructure/llm/gateway_factory.py`, adapters, dependencies, generation/RAG/memory call sites |
| BE-03 Concurrency/state | P2 — **PASS** | Backend | PostgreSQL user/emotion mutations và Redis guild/channel transitions atomic; renewable owner-fenced locks; stale summary/cache writes fenced bằng monotonic revisions; cache publish sau commit qua BE-01 outbox; replay/rollback/concurrency và TD-035 lifecycle verification đạt trên isolated PostgreSQL/Redis. Evidence: `reports/BE03_Concurrency_State_Verification.md`. | locks, cache, persistence, community pipeline, maintenance supervisor |
| OPS-02 OpenTelemetry/SLO | P2 — **PASS** | MLOps | Backend-neutral OpenTelemetry spans cover HTTP/SSE, pipeline/stages, retrieval/rerank, LLM provider/generation, grounding, worker jobs and readiness dependencies. Typed allowlisted dimensions prevent content/PII/high-cardinality IDs; opt-in 5% sampling, bounded batch queue/timeouts and non-fatal delegation protect the request path. Seven OpenSLO objectives preserve approved `NFR-REL-001`, `NFR-PERF-002/003/006` thresholds; three fast/slow burn pairs plus actionable provider, queue, DLQ, dependency, leakage, cross-tenant, index-drift and call-budget cost-control alerts are versioned with three minimal dashboards. Provider-reported tokens are observable; currency cost remains explicitly unavailable until an authoritative versioned price registry exists. Evidence: `reports/OPS02_Observability_Verification.md`. | `app/domain/interfaces/observability.py`, `app/infrastructure/observability/`, middleware, pipeline, retriever, LLM gateway, worker, health, `deploy/observability/` |
...
| DB-01 Single schema owner | P2 — **PASS** | Backend/Data | Alembic là production PostgreSQL schema owner duy nhất; app/Discord không runtime DDL; startup revision guard, CI drift check, forward/rollback/backup runbook và isolated migration verification đạt. Evidence: `reports/DB01_Schema_Ownership_Verification.md`. | `alembic_migrations`, engine, Discord DB init |
