from collections.abc import AsyncIterator
from datetime import datetime

import pytest

from app.application.ingestion.sync_strategies.all_pages_sync import AllPagesSyncStrategy
from app.domain.entities.wiki import WikiPage


class MockWikiClient:
    async def get_all_pages(self) -> AsyncIterator[WikiPage]:
        yield WikiPage(
            page_id=1,
            title="Page1",
            latest_revision_id=10,
            last_updated=datetime.utcnow(),
        )
        yield WikiPage(
            page_id=2,
            title="Page2",
            latest_revision_id=20,
            last_updated=datetime.utcnow(),
        )


class MockSyncRepo:
    async def get_latest_revision_id(self, page_id: int) -> int | None:
        if page_id == 1:
            return 10  # Up to date
        if page_id == 2:
            return 15  # Out of date
        return None


@pytest.mark.asyncio
async def test_all_pages_sync_strategy_keeps_versioned_build_complete() -> None:
    client = MockWikiClient()
    repo = MockSyncRepo()
    strategy = AllPagesSyncStrategy()
    
    pages_to_sync: list[WikiPage] = []
    async for page in strategy.enumerate_pages_to_sync(client, repo):
        pages_to_sync.append(page)

    assert [page.page_id for page in pages_to_sync] == [1, 2]
    assert [page.latest_revision_id for page in pages_to_sync] == [10, 20]
