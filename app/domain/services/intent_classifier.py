...
class IntentClassifier:
    ...
    def is_small_talk(cls, message: str) -> bool:
        ...
        info_words = {"là gì", "ở đâu", "khi nào", "tại sao", "như thế nào", "ai là", "vũ khí", "chiêu", "kỹ năng", "tìm kiếm", "tra cứu"}
    ...
    def determine_routing_and_rewrite(
        ...
        """
        Dual-Signal Decision Matrix:
        Evaluates Semantic Intent + Linguistic Coreference Signals to decide:
          - BYPASS: 0 tokens, 0ms (Skip RAG & Rewrite for Small-Talk)
          - LLM_REWRITE: DeepSeek V4 Flash (~40 tokens, ~100ms) for multi-turn coreference/follow-ups
          - FAST_PATH: 0 tokens, < 0.2ms (Entity Alias Enrichment directly to Qdrant)
        """
        ...
        # 2. Coreference Pronoun / Follow-up Marker Gate -> LLM_REWRITE
        if has_history and has_coreference_markers(user_message):
            return {
                "decision": "LLM_REWRITE",
                "needs_llm_rewrite": True,
                "reason": "Coreference/Pronoun markers detected in multi-turn conversation"
            }

        # 3. Short Follow-up Ambiguity Gate (< 4 words with history) -> LLM_REWRITE
        words = cleaned_query.split()
        if has_history and len(words) <= 3 and intent_result.confidence < 0.85:
            return {
                "decision": "LLM_REWRITE",
                "needs_llm_rewrite": True,
                "reason": "Short follow-up query requiring context disambiguation"
            }

        # 4. Default Standalone Gate -> FAST_PATH (0 Token, Entity-Enriched)
        return {
            "decision": "FAST_PATH",
            "needs_llm_rewrite": False,
            "reason": "Standalone query with high confidence / explicit entities"
        }
